...
export default function DeleteTeamButton({
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/dashboard/team/${memberSlug}`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Erro ao deletar membro:', result.error);
        alert('Erro ao deletar membro: ' + (result.error || 'Erro desconhecido'));
      }
    } catch (error) {
        ...
        <div className='flex flex-col items-center text-center gap-4'>
          ...
          <div>
            ...
            <p className='text-sm text-gray-600'>
              O membro <span className='font-semibold'>{memberName}</span> será
              removido permanentemente da equipe.
            </p>
          </div>
        </div>
      </YVModal>
