...
interface EstudanteOptionsProps {
...
export default function EstudanteOptions({
  register,
  errors,
  watch,
  setValue,
  ...
  etapaAtual,
  totalEtapas
}: EstudanteOptionsProps) {
  const [selectedOpcao, setSelectedOpcao] = useState<string>('');

  const handleOpcaoSelect = (opcao: string) => {
    setSelectedOpcao(opcao);
    setValue('estudanteOpcao', opcao);
  };

  const opcoes = [
    {
      id: 'formacao-exterior',
      icon: 'study-icon',
      title: 'Conseguir uma formação que abra portas lá fora'
    },
    {
      id: 'morar-fora-estudando',
      icon: 'book',
      title: 'Morar fora estudando e aproveitar cada momento'
    },
    {
      id: 'pos-mestrado',
      icon: 'finance-house',
      title: 'Fazer uma pós ou mestrado em uma instituição de respeito'
    },
    {
      id: 'explorando-opcoes',
      icon: 'explore-travel',
      title: 'Ainda tô explorando as opções'
    }
  ];
