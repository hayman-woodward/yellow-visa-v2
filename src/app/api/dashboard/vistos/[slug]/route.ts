...
export async function GET(
  ...
  try {
    ...
    if (!visto) {
      return NextResponse.json({ error: 'Visto not found' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    if (validated.slug !== slug) {
      const slugExists = await prisma.visto.findUnique({
    ...
    const visto = await prisma.visto.update({
      where: { slug },
      data: {
        ...
        // Campos dos requisitos especiais
        requisitosEnabled: validated.requisitosEnabled,
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to update visto' },
      { status: 500 }
    );
  }
}

// DELETE - Deletar visto
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to delete visto' },
      { status: 500 }
    );
  }
}
